...
function population(p0, percent, change, allpopula) {
  let newpop = p0;
  let years = 0;
  // אנו מאתחלים שני משתנים חדשים שיקבלו ערכים בכל איטרציה
  // מספר השנים עולה ב1 עם כל עליה באוכלוסיה על פי נוסחה
  while (newpop < allpopula) {
    newpop = newpop + newpop * (percent / 100) + change;
    years += 1;
  }
  return console.log(years);
}
// (p0,percent,change/Aug,popula)
population(1500, 5, 100, 5000);
// population(1000, 2, 50, 1200)
// population(100, 5, 100, 500)
//      newpop = newpop*(percent/100)+change ;
