...
const listOfpassengers = [
    ...
    const bus = function () {
        let peoplein = 0;
        let peopleout = 0;
      for (let index = 0; index < listOfpassengers.length; index++) {
        
        for (let a = 0; a < listOfpassengers[index].length; a++) {
          if (0 === a) {
            peoplein = peoplein + listOfpassengers[index][a];
            console.log(`peoplein is ${peoplein}`);
            

          } else {
            peopleout = peopleout + listOfpassengers[index][a];
            console.log(`peopleout is ${peopleout} `);
        
          }
          
        }
      };
      return console.log(peoplein - peopleout)
    };
  
bus(listOfpassengers);